...
async function main() {
  ...
  console.log("Creating 16 tables...");
  for (const stmt of creates) {
    await sql.query(stmt);
  }

  console.log("Done! Verifying...");
  const verify = await sql.query(`SELECT tablename FROM pg_tables WHERE schemaname = 'public' ORDER BY tablename`);
  console.log("Tables:", (verify as any).rows.map((r: any) => r.tablename).join(", "));
}
